...
class CrosswordCreator():
        ...
        for var in assignment:
            ...
            for neighbor_var in self.crossword.neighbors(var):
                if neighbor_var in assignment:
                    ...
                    if assignment[var][i] != assignment[neighbor_var][j]:
                        return False
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables - set(assignment):
            if (
                opt is None or
                len(self.domains[var]) < len(self.domains[opt]) or
                len(self.crossword.neighbors(var)) > len(self.crossword.neighbors(opt))
            ):
                opt = var
